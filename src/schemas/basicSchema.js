...
export const basicSchema = yup.object().shape({
  ...
  address: yup.string().min(10, "Direccion es muy corta").required("Campo direccion es obligatorio"),
  phoneNumber: yup
    .string()
    .matches(/^\+\d{1,3} \d{7,}$/, "Telefono debe estar en el formato +[codigo de pais] [numero]")
    .required("Campo telefono es obligatorio"),
  region: yup
    .string()
    .matches(/^[A-Za-z\s]+$/, "Region no puede contener números ni caracteres especiales")
    .required("Campo region es obligatorio"),
  city: yup
    .string()
    .matches(/^[A-Za-z\s]+$/, "Ciudad no puede contener números ni caracteres especiales")
