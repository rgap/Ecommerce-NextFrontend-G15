...
export default function CartShipping() {
  ...
  const totalCompra = (total + (costoEnvio !== "Por definir" ? parseFloat(costoEnvio) : 0)).toFixed(2);
      ...
      <section className="cart-info-left lg:w-[60%] flex flex-col min-w-[590px]">
        ...
        <div className="mx-10 xl:mx-20 my-0">
          <div className="grid-shipping">
            <Breadcrumb />
            <p className="text-xl text-center md:text-left font-bold capitalize leading-8 break-words mb-5 mt-8">Opciones de Envio</p>
            ...
            <div className="flex justify-center max-w-[520px] flex-col mb-10 gap-2 px-10 w-full m-auto">
              <div className="flex flex-col gap-5 self-center">
                ...
                <div className="grid grid-cols-[20px_1fr_0.6fr] gap-y-3 items-center gap-0 md:gap-8">
                  <input type="radio" value="regular" checked={selectedValue === "regular"} onChange={handleRadioChange} className="w-4 h-4" />
                  <div className="font-bold text-lg ml-4">
                    Envio Regular
                    <p className="font-semibold text-sm">5 a 7 días hábiles</p>
                  </div>
                  <div className="text-lg ml-5 md:ml-0 text-center">S/ 12.00</div>
                </div>

                {/* Envio Rapido */}
                <div className="grid grid-cols-[20px_1fr_0.6fr] gap-y-3 items-center gap-0 md:gap-8">
                  <input type="radio" value="rapido" checked={selectedValue === "rapido"} onChange={handleRadioChange} className="w-4 h-4" />
                  <div className="font-bold text-lg ml-4">
                    Envio Rapido
                    <p className="font-semibold text-sm">2 a 3 días hábiles</p>
                  </div>
                  <div className="text-lg ml-5 md:ml-0 text-center">S/ 20.00</div>
                </div>
              </div>

              <div className="flex justify-center mt-20">
                <div className="w-full flex justify-between">
                  <div className="flex w-[190px] h-[40px] items-center gap-3 cursor-pointer" onClick={redirect("/cart-info")}>
                    <Image
                      className="w-6 h-6"
                  ...
                  <div className="flex w-[190px] h-[40px] justify-center items-center flex-shrink-0">
                    <Button ruta="/cart-payment" text="Continuar con Pago" type="button" />
                  </div>
                </div>
              </div>
            </div>
