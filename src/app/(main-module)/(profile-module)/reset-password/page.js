...
export default function ResetPassword() {
  ...
  const [isConfirmationLinkClicked, setIsConfirmationLinkClicked] = useState(false);
  ...
  const handleResetPasswordFormSubmit = async event => {
    ...
    if (validateResetPasswordForm()) {
      ...
      const user = getUserByEmail(resetValues.email.toLowerCase());
      if (user) {
        ...
        {isFormSubmitted && !isConfirmationLinkClicked ? (
          <div className="text-center">
            <span className="text-neutral-950 text-base leading-loose">
              Si es que existe una cuenta para beautipol.alpha.1@gmail.com, recibirás un correo electrónico con instrucciones para restablecer su
              contraseña. Si no te llega, asegúrate de revisar tu carpeta de spam.
            </span>
            <br />
            <a href="#" onClick={handleConfirmationLinkClick} className="mt-4 block">
              Enlace enviado al correo
            </a>
          </div>
        ) : isConfirmationLinkClicked ? (
          <form autoComplete="off" onSubmit={handleResetPasswordFormSubmit} className="mb-5 flex flex-col gap-3">
            <div className="text-center mb-5">
              <span className="text-neutral-950 text-base">Ingresa tu nueva contraseña</span>
            </div>
            ...
            {inputsPasswordReset.map(input => (
              <div key={input.name}>
                ...
                />
                <span className="text-red-500 mt-1 text-xs">{resetPasswordErrors[input.name]}</span>
              </div>
            ))}
          ...
          <form onSubmit={handleResetFormSubmit} autoComplete="off" className="mb-5 flex flex-col gap-3">
            <div className="text-center mb-5">
              <span className="text-neutral-950 text-base">Ingresa tu correo electrónico para cambiar tu contraseña</span>
            </div>
            ...
            {inputsReset.map(input => (
              <div key={input.name}>
                ...
                />
                <span className="text-red-500 mt-1 text-xs">{resetErrors[input.name]}</span>
              </div>
            ))}
